...
#logistic regression and svm are for binary classification data - only a yes and no, membership or not membership
#we have more than that, so wed have to make a bunch of those for each class (membership or not)
...
knn_best_fit %>% #look at what else you can get out of this
  extract_fit_engine()
...
write.csv(submission, "knn_submission.csv", row.names = FALSE)
...
write.csv(submission, "dt_submission.csv", row.names = FALSE)
...
dt_tune_results <- rf_wf %>%
  tune_bayes(
    resamples = train_folds,
    metrics = metric_set(mn_log_loss),
    initial = 3,  #make this bigger after it works
    control = control_bayes(parallel_over = "everything")
  )
